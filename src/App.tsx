    ...
    return guesses.map((guess, i) => {
      return (
        <div key={i} className="flex">
          <span className="bg-yellow-400 hover:bg-yellow-300 rounded-lg p-1 mr-2 my-1 text-blue-800 text-xs font-semibold inline">
            {i + 1}
          </span>
          {renderGuessCard("Composer", guess.composer)}
        </div>
      );
    });
  };

  const renderGuessCard = (title: string, text: string): ReactNode => (
    <div className="block max-w-[18rem] rounded-lg bg-white text-left shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
      <div className="p-2">
        ...
        <p className="text-sm leading-normal text-neutral-600 dark:text-neutral-200">
          {text}
        </p>
      </div>
    </div>
  ...
  return (
    <>
      <div className="container mx-auto">
        ...
        <img className="mx-auto" src={music1} />
        <div className="input-container bg-gray-300">
